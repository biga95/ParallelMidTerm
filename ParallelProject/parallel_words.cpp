...
#pragma omp for schedule(static)
	...
	// STAMPA TEMPI
	std::cout << "\nTEMPI:\n";
	std::cout << "Bigrammi parole (parallel):   " << time_big << " s\n";
	std::cout << "Trigrammi parole (parallel):  " << time_tri << " s\n";
	std::cout << "Bigrammi char (parallel):     " << time_big_char << " s\n";
	std::cout << "Trigrammi char (parallel):    " << time_tri_char << " s\n";
	std::cout << "Thread usati: " << num_threads << "\n";

	// TOP 10

	std::vector<std::pair<std::string, int>> v1;
	for (auto& p : bigrams) v1.push_back(p);
	std::sort(v1.begin(), v1.end(), [](const auto& a, const auto& b) { return a.second > b.second; });
	std::cout << "\nTop 10 bigrammi parole:\n";
	for (int i = 0; i < 10 && i < (int)v1.size(); i++)
		std::cout << v1[i].first << " = " << v1[i].second << "\n";

	std::vector<std::pair<std::string, int>> v2;
	for (auto& p : trigrams) v2.push_back(p);
	std::sort(v2.begin(), v2.end(), [](const auto& a, const auto& b) { return a.second > b.second; });
	std::cout << "\nTop 10 trigrammi parole:\n";
	for (int i = 0; i < 10 && i < (int)v2.size(); i++)
		std::cout << v2[i].first << " = " << v2[i].second << "\n";

	std::vector<std::pair<std::string, int>> v3;
	for (auto& p : big_char) v3.push_back(p);
	std::sort(v3.begin(), v3.end(), [](const auto& a, const auto& b) { return a.second > b.second; });
	std::cout << "\nTop 10 bigrammi char:\n";
	for (int i = 0; i < 10 && i < (int)v3.size(); i++)
		std::cout << v3[i].first << " = " << v3[i].second << "\n";

	std::vector<std::pair<std::string, int>> v4;
	for (auto& p : tri_char) v4.push_back(p);
	std::sort(v4.begin(), v4.end(), [](const auto& a, const auto& b) { return a.second > b.second; });
	std::cout << "\nTop 10 trigrammi char:\n";
	for (int i = 0; i < 10 && i < (int)v4.size(); i++)
	...
	{
		std::ofstream out(par_file);
		if (!out) {
			std::cout << "Errore apertura file " << par_file << "\n";
		}
		else {
